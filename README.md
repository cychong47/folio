...
- **EXIF timezone preservation** — photo curation prefers original/base Photos resources, shows selected-event timestamp and date-filter diagnostics, honors `OffsetTimeOriginal`, `SubSecTimeOriginal`, and inline EXIF timezone offsets, and falls back to Photos' own date when timezone-less library JPEGs lack or disagree with GPS-derived timezone evidence
