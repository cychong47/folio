...
- **EXIF timezone preservation** — photo curation prefers original/base Photos resources, shows selected-event timestamp diagnostics, honors `OffsetTimeOriginal`, `SubSecTimeOriginal`, and inline EXIF timezone offsets, falls back to Photos' own date when timezone-less library JPEGs lack or disagree with GPS-derived timezone evidence, and infers a display timezone from the curated batch when possible
...
- **Photo Curation Workspace** — scan a vacation photo folder (`⌘K`); Photos-library curation prefers original/base asset resources for metadata, shows the selected event's resource and timestamp decision in the sidebar, uses capture-local date and time from separated EXIF offsets, subsecond EXIF fields, inline EXIF offsets, GPS-derived timezones that agree with Photos, or Photos' own date plus an inferred batch display timezone for ambiguous library JPEGs, sorts and numbers event rows by the displayed capture time, and keeps the date range editable from the curation toolbar
