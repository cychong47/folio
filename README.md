...
- **Photo Curation Workspace** — scan a vacation photo folder (`⌘K`); photos and event rows use capture-local date and time from EXIF or GPS-derived timezones, auto-grouped into events by time and location, and the side-by-side date range picker can be updated from the curation toolbar while curation stays open
