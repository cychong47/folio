...
- **EXIF timezone preservation** — photo curation prefers original/base Photos resources, shows curation timestamp diagnostics, and honors `OffsetTimeOriginal`, `SubSecTimeOriginal`, and inline EXIF timezone offsets so no-GPS photos display in their capture timezone instead of the Mac's current timezone
...
- **Photo Curation Workspace** — scan a vacation photo folder (`⌘K`); Photos-library curation prefers original/base asset resources for metadata, shows the resource and timestamp decision in the sidebar, uses capture-local date and time from separated EXIF offsets, subsecond EXIF fields, inline EXIF offsets, or GPS-derived timezones, sorts and numbers event rows by the displayed capture time, and keeps the date range editable from the curation toolbar
