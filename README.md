...
- **EXIF timezone preservation** — photo curation honors `OffsetTimeOriginal`, including fractional-second timestamps, so no-GPS photos display in their capture timezone instead of the Mac's current timezone
...
- **Photo Curation Workspace** — scan a vacation photo folder (`⌘K`); photos and event rows use capture-local date and time from EXIF offsets, fractional EXIF offsets, or GPS-derived timezones, event rows sort and number by the displayed capture time, and the side-by-side date range picker can be updated from the curation toolbar while curation stays open
