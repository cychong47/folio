...
- **Photo Curation Workspace** — scan a vacation photo folder (`⌘K`); photos and event rows use capture-local date and time from EXIF or GPS-derived timezones, event rows sort by the displayed capture time, and the side-by-side date range picker can be updated from the curation toolbar while curation stays open
