...
- **EXIF timezone preservation** — curation prefers original/base Photos resources, shows date-filter diagnostics for ambiguous Photos-library rows, honors `OffsetTimeOriginal`, `SubSecTimeOriginal`, and inline EXIF timezone offsets, uses original metadata GPS when PhotoKit omits location, supports manual UTC offsets for no-GPS/no-timezone Photos, trusts camera-local EXIF time when GPS is absent, and falls back to Photos' own date when EXIF is missing or GPS-assumed EXIF conflicts with Photos
