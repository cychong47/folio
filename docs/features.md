...
- **Capture-local date filtering** — Photos-library date ranges match the capture-local day shown in curation, using EXIF offsets, fractional EXIF offsets, or GPS-derived timezones so previous-day and next-day timezone offsets do not leak into the selected day
...
- **EXIF timezone preservation** — folder-scanned curation photos honor `OffsetTimeOriginal`, including fractional-second timestamps, keeping no-GPS photo timestamps aligned with GPS-backed photos from the same shoot
